...
public class FirstLeveCreate : MonoBehaviour {

    public GameObject mountain;
    public Vector3 mountainV3;
    public Quaternion mountainQ;
    public GameObject otherTown;
    public GameObject otherTown1;
    public GameObject otherTown2;
    public GameObject otherTown3;
    public GameObject otherTown4;
    public GameObject otherTown5;
    public GameObject finish;
    public GameObject town;
    private bool townVisible = false;
    public Player p;

    // Use this for initialization
    void Start () {
        /*for (int i = 0; i < 5; i++) {
            SimplePool.Spawn(mountain, mountainV3, mountainQ);
            mountainV3.x = mountainV3.x + 2f;
        }*/
	}
    ...
    private void fogOfWar(GameObject go) {
        Debug.Log("FOG OF WAR");
        const int RADIUS_FOG = 5;
        float distance = (float)System.Math.Sqrt((go.transform.position.x - town.transform.position.x) * (go.transform.position.x - town.transform.position.x)
            + (go.transform.position.y - town.transform.position.y) * (go.transform.position.y - town.transform.position.y));
        if (distance < RADIUS_FOG) {
            Debug.Log("FOG OF WAR : TRUE");
            go.gameObject.SetActive(true);
            var renderer = go.GetComponent<Renderer>();
            renderer.material.shader = Shader.Find("Transparent/Diffuse");
            if (distance > 4) {
                renderer.material.color = Color.white * ((RADIUS_FOG - distance)); // белый с 25% не прозрачности
            }
            else {
                if (!townVisible) {
                    p.sirena();
            ...
            Debug.Log("FOG OF WAR : FALSE");
            if (!townVisible) {
                go.gameObject.SetActive(false);
            } else {
                var renderer = go.GetComponent<Renderer>();
                renderer.material.shader = Shader.Find("Transparent/Diffuse");
                renderer.material.color = Color.white * 0.4f; // белый с 25% не прозрачности
            }
        }
    }
}
